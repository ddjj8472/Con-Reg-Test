...
def apply_custom_style(is_dark: bool):
    ...
    /* 🔥 하단 채팅 입력창(st.chat_input) 다크 모드 완벽 적용 */
    [data-testid="stChatInput"] {{ background-color: {c['bg']} !important; }}
    [data-testid="stChatInput"] > div {{ background-color: {c['card']} !important; border: 1px solid {c['border']} !important; }}
    [data-testid="stChatInput"] textarea {{ color: {c['txt']} !important; background-color: transparent !important; -webkit-text-fill-color: {c['txt']} !important; }}
    [data-testid="stChatInput"] svg {{ fill: {c['txt']} !important; color: {c['txt']} !important; }}
    </style>
