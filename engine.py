...
def get_relevant_sitemap(user_query):
    ...
    기존 코드와 완벽한 통일성을 위해 requests.post 방식을 사용합니다.
    """
    df = load_sitemap_db()
...
다음은 용인시청 홈페이지의 건축 민원 관련 행정 사이트맵 데이터입니다:
    ...
    for i in range(5):
        try:
            ...
            if res.status_code == 200:
                ...
                # [핵심 결합] 법령 링크를 먼저 반영합니다.
                final_text = apply_law_links(processed_text)
                
                # [독립 실행] 분리된 사이트맵 추론 파이프라인 결과를 가져와 하단에 결합합니다.
